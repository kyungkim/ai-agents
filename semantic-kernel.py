...
async def process_expenses_data(prompt, expenses_data):
    ...
    async with (
        ...
        expenses_agent_def = await project_client.agents.create_agent(
            ...
            When a user submits expenses data and requests an expense claim, use the plug-in function to send an email
            to expense@contoso.com with the subject 'Expense Claim' and a body that contains itemized expenses with 
            a total. Then confirm to the user that you've done so.
            """
        )
